...
(define crearMatriz
 (lambda (N M board x y ships)
   (if (= N x) board
           (cond
             ((and (< y M) (not(null? ships)))
              (if (not(char?(car(reverse(buscar x y ships)))))
                  (crearMatriz N M (append board (list(list x y #\-))) x (+ y 1) ships)
                    (crearMatriz N M (append board (list(list x y (car(reverse(buscar x y ships)))))) x (+ y 1) (removeShip x y ships))))
             ((and (< y M) (null? ships))
              (crearMatriz N M (append board (list(list x y #\-))) x (+ y 1) ships))   
...
;(display M)
;(cons 1 2)
;(char?(car(reverse(buscar 0 4 (agregarShips '() ships 10 10 0)))))
;(agregarShips '() ships 10 10 0)
