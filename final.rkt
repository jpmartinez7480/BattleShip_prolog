...
;entrada:t:lista vacia donde se guardan las posiciones de los barcos a agregar, L: lista con los barcos a agregar, N M: fila y columna del tablero, cnt: acumulador que indica cuantas casillas lleva
;salida: lista de posiciones a distribuir los barcos
(define agregarShips
  (lambda (t L N M cnt)
    (if(null? L)
       t
...
;funcion que remueve un barco de la lista
;entrada: lista: lista de barcos, p:posicion del barco a eliminar
;salida:lista de barcos sin el barco que tiene la posicion p
(define removeShip
  (lambda (p lista)
    (if (null? lista) '()
...
;entrada:p:coordenada a buscar, lista: lista de barcos
;salida:lista sin la coordenada del barco y en la ultima posicion el barco eliminado
(define buscar
  (lambda (p lista)
    (if(null? lista) '()
...
;entrada:lista con los barcos y coordenadas, p: coordenada a buscar
;salida: valor booleano que indica si la coordenada p ya esta en la lista, #\t en caso de que se encuentre.
...
;funcion auxiliar que permite crear una lista posiciones y barcos usando recursion lineal
;entrada: N: cantidad de filas, M: cantidad de columnas, p: position actual, ships: lista de barcos a agregar
;salida: lista de posiciones y barcos creada usando recursion lineal
(define crearMatrizL
  (lambda (N M p ships)
    (if(= N (position-x p)) '()
       (cond
...
;funcion que pormite crear un tablero usando recursion lineal
;entrada:N M: dimensiones del tablero,ships: barcos a agregar, seed:semilla para poner los barcos aleatorios de la cpu.
;salida:lista de listas quue contiene la configuracion del tablero.
...
;funcion que realiza una jugada en la partida
;entrada:board:lista de listas que contiene la configuracion del tablero,ship:barco del usuario que realizara el ataque,positions:lista de coordenadas donde
;se realizara el ataque, seed: semilla que permite generar un ataque aleatorio por parte de la cpu.
;salida: board:lista de listas que contiene la nueva configuracion del tablero luego de que el usuario y la cpu hayan realizado un ataque.
(define play
...
;funcion auxiliar que crea coordenadas de los barcos a colocar por parte del usuario
;entrada:position:posicion inicial a distribuir, ship: barco a colocar, L: lista vacia donde se guardan las posiciones del barco a poner.
;salida: lista que contiene las posiciones de barcos a poner.
(define crearCoordenadasPutShip
  (lambda (position ship L)
    (if(null? position) L
       (cond
         ((eq? ship #\d) (crearCoordenadasPutShip (cdr position) ship (append L (list (make-position (position-x position) (position-y position))))))
         ((eq? ship #\s) (crearCoordenadasPutShip (cdr position) ship (append L (Submarino #\s position 0 '() 0))))
         ((eq? ship #\a) (crearCoordenadasPutShip (cdr position) ship (append L (acorazado #\a position 1 '() 0))))
         )
       )
    )
  )
    ...
    (let((x (coordenada-y (reverse board))))
      (fold-left
       (lambda (base argumento)
         (if(= x (car(cdr argumento))) (string-append base " " (make-string 1 (caracter argumento)) "\n")
            (cond
              ((= showComplete 1)
               (string-append base " " (make-string 1 (caracter argumento))))
              ((= showComplete 0)
               (if(< (car argumento) (/ (+ x 1) 2))
                  (if(and(not(eq? (caracter argumento) #\-)) (not(eq? (caracter argumento) #\x)))
...
;funcion que recibe un tablero para obtener su representacion en xml
;entrada: board: lista de listas que contiene la configuracion del tablero.
;salida: string que representa un board en formato xml
(define board->xml
  (lambda (board)
    (let((x (coordenada-y (reverse board))))
      (fold-left
       (lambda (base argumento)
         (cond
           ((and(= x (car(cdr argumento)))(= x (car argumento)) (string-append base " " (make-string 1 (caracter argumento)) "</fila></board>"))) 
           ((= x (car(cdr argumento))) (string-append base " " (make-string 1 (caracter argumento)) " </fila>"))
           ((= (car(cdr argumento)) 0) (string-append base "<fila>" (make-string 1 (caracter argumento))))
           (else (string-append base " " (make-string 1 (caracter argumento))))
         )
       )
       (string-append "<board><fila>" (make-string 1 (caracter(casilla-board board))))
       (cdr board)
      )
    )
  )
...
;funcion que recibe un board para obtener su representacion en json
;entrada: board: lista de listas que contiene la configuracion del tablero
;salida: string representativo de board en representacion json
(define board->json
  (lambda (board)
    (let ((x (coordenada-y (reverse board))))
      (fold-left
       (lambda (base argumento)
         (cond
           ((and(= x (car(cdr argumento))) (= x (car argumento)) (string-append base " " (make-string 1 (caracter argumento)) "\"" "}}")))
           ((= x (car(cdr argumento))) (string-append base " " (make-string 1 (caracter argumento)) " \" "","))
           ((= (car(cdr argumento)) 0) (string-append base "\"" "fila" (number->string (car argumento)) "\""  ":" "\"" (make-string 1(caracter argumento)) ))
           (else (string-append base " " (make-string 1 (caracter argumento))))
           )
         )
       (string-append "{\"" "board" "\"" ":{" "\"" "fila0" "\"" ":" "\"" (make-string 1 (caracter(casilla-board board))))
       (cdr board)
       )
      )
    )
...
;funcion que prepara la jugada del usuario.
;entrada:board:lista de listas que contiene la configuracion del tablero, ship: barco del usuario que realizara el ataque, positions: lista de position que
;contiene las coordenadas a atacar de la cpu, b: lista vacia donde se guarda el nuevo tablero luego de un ataque, seed: semilla que permite realizar un ataque
;aleatorio.
;salida:nueva lista de lista que contiene el tablero modificado segun el ataque del usuaro. 
...
;entrada: board:lista de listas que contiene la configuracion del tablero, seed:semilla que permite actualizar el random,L:lista vacia donde se guarda el tablero
;modificado, positions: lista de positions donde se encuentran las casillas a atacar.
;salida:nueva lista de listas que contiene la nueva configuracion del tablero,luego de que la cpu haya realizado su ataque.
...
;funcion que calcula las posiciones de ataque.
;entrada: board:lista de listas que contiene la configuracion del tablero, ship:barco que realiza el ataque,seed: semilla que permite generar una casilla aleatoria
;L: lista vacia donde se guardan las posiciones a ser atacadas, positions: position inicial, n: entero que indica si se esta preparando un ataque para la cpu
;o usuario
;salida: lista de posiciones a atacar
...
;funcion que permite configurar el ataque de un submarino
;entrada: board: lista de listas que contiene la configuracion de un tablero, x e y: coordenadas iniciales para configurar un ataque, cnt: acumulador que cuenta
;las casillas seleccionadas a atacar, L:lista vacia donde se guarda las posiciones a atacar, n: entero que indica si el ataque lo realiza el pc o un usuario
;salida: lista de posiciones a atacar por un submarino
...
;funcion que permite configurar el ataque de un acorazado
;entrada: board: lista de listas que contiene la configuracion de un tablero, x e y: coordenadas iniciales para configurar un ataque, cnt: acumulador que cuenta
;las casillas seleccionadas a atacar, L:lista vacia donde se guarda las posiciones a atacar, n: entero que indica si el ataque lo realiza el pc o un usuario
;salida: lista de posiciones a atacar por un acorazado
...
;funcion que permite conocer los barcos disponibles para el usuario o el pc
;entrada:board:lista de listas que contiene la configuracion del tablero, cnt: acumulador que permite saber en que parte del tablero se encuentra,
;dim: lugar maximo dentro de board a buscar barcos, L: lista vacia donde se guardan los barcos actuales disponibles
...
;funcion que le permite a la cpu escoger un barco aleatorio para atacar.
;entrada:ships:lista de barcos actuales disponibles,seed:semilla que permite generar un numero aleatorio.
;salida:barco escogido para realizar un ataque.
...
;funcion que permite obtener la posicion de un barco
;entrada: ships:lista de barcos, i: posicion del barco a buscar.
;salida:barco escogido para atacar.
(define getByPos
  (lambda (ships i)
    (if(= i 0) (car ships)
...
(define pos (crearCoordenadasPutShip p #\s '()))
