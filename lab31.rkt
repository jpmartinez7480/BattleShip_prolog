...
(define play
  (lambda (board ship positions seed)
    (if(not(null? positions))
             ...
             (cond
               ((eq? #\- (caracter(casilla-board board)))
                   (if(= (length positions) 1)
                      (append (list(list(coordenada-x board) (coordenada-y board) #\x)) (play (cdr board) ship (cdr positions) seed) );jugada
               ...
               ((eq? #\a (caracter(casilla-board board)))
                (if(= (length positions) 1) 
                   (append (list(list(coordenada-x board) (coordenada-y board) #\x )) (play (cdr board) ship (cdr positions) seed) );juagda
                   (append (list(list(coordenada-x board) (coordenada-y board) #\x )) (play (cdr board) ship (cdr positions) seed))
                   )
                )
               )
       ...
       ;(append board (cpuAttack board seed '() #\g '()) ) 
       board
       )
   )
  )
...
(define cpuAttack
  (lambda (board seed p ship b)
    (let ((p (prepareToAttack board (+ seed 123) '())))
      (append b (play(board ship p (+ seed 456))))
      )
    )
  )

(define prepareToAttack
  (lambda (board seed L)
    (let ((barcos (barcosActuales board 0 (/ (length board) 2) '())))
      (let((ship (chooseShipToAttack barcos (+ seed 1234))))
         (let ((positions (positionCpuAttack ship (+ seed 2312) board '())))
                (play board ship positions (+ 1242445 seed))
               )
            )
          )
         )
            ...
            ((eq? ship #\d)
             (let(( x (getListaRandom 1 (+ 2435 seed) (/ (+ (getRows board) 1) 2))))
               (let (( y (getListaRandom 1 (+ seed 732723) (getCol board))))
                 (positionCpuAttack ship seed board (append  L (list(make-position (+ (car x) 5) (car y)))))
             )
           )
          )
            ((eq? ship #\s)
             (let(( x (getListaRandom 1 (+ 2235 seed) (/ (+ (getRows board) 1) 2))))
               (let (( y (getListaRandom 1 (+ seed 2723) (getCol board))))
                 (positionCpuAttack ship seed board (append  L (AtaqueSubmarino board (+ (car x) 5) (car y) 0 '())))
                 )
               )
             )
            ((eq? ship #\a)
             (let((x (getListaRandom 1 (+ 2235 seed) (/ (+ (getRows board) 1) 2))))
                (let (( y (getListaRandom 1 (+ seed 2723) (getCol board))))
                  (positionCpuAttack ship seed board (append  L (AtaqueAcorazado board (+ (car x) 5) (car y) 0 '())))
                 )
            )
          )
          )
...
(define AtaqueAcorazado
  (lambda(board x y cnt L)
    (if(< cnt 2)
       (cond
         ((= cnt 0) (AtaqueAcorazado board x y ( + 1 cnt) (append L (list(make-position x y)))))
         ((= cnt 1)
          (if(>(+ y 2) (getRows board))
